...
class TestModel(unittest.TestCase):
    def test_two_inputs_one_output(self):
        data = pd.read_csv('data_2_db_1.csv')
        m1, b1 = -1, 10
        ...
        mod = Model([2, Layer(5, activation="sigmoid"), Layer(1, activation="sigmoid")])
        mod.compile(loss='binary_crossentropy', input_normalization=Normalization.z_score)
        mod.fit(X, Y, batch_size=5, epochs=10)
        no_fails = 0
        fail = []
        ...
        #print("first layer: ", mod._layers[1].W)
